...
def generate_report(farmer_name):
	...
	for period in periods:
		...
		if total_profits < 0:
			emoji = "🍅"  # Tomato emoji for negative profits
		else:
			emoji = "🌽"  # Corn emoji for positive profits

		table_data.append([f"Total Sales ({period.capitalize()})", f"₱{total_sales:,.2f}"])
		table_data.append([f"Total Expenses ({period.capitalize()})", f"₱{total_expenses:,.2f}"])
		table_data.append([f"Total Profits ({period.capitalize()})", f"₱{total_profits:,.2f} {emoji}"])

	# Add the overall totals to the table
	table_data.append(["Overall Total Sales", f"₱{overall_sales:,.2f}"])
	table_data.append(["Overall Total Expenses", f"₱{overall_expenses:,.2f}"])
	table_data.append([f"Overall Total Profits", f"₱{overall_profits:,.2f}"])

	# Personal greeting and report header
	print(f"Hello Farmer {farmer_name}, here are your profit calculations:")
